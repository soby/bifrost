...
func (bifrost *Bifrost) getProviderQueue(providerKey schemas.ModelProvider) (*ProviderQueue, error) {
	...
	if config == nil {
		...
		// Auto-initialised providers share one HTTP client per built-in provider
		// type, so construction-time settings cannot encode request-specific
		// policy. Build the client with a transport-level safety ceiling instead;
		// the effective timeout is enforced per request from
...
// getter. Request execution uses getProviderQueue, which also handles
...
func executeRequestWithRetries[T any](
	...
	for attempts = 0; attempts <= effectiveConfig.NetworkConfig.MaxRetries+extraAttempts; attempts++ {
		...
		if attempts > 0 {
			...
			logger.Debug("retrying request (attempt %d/%d) for model %s: %s", attempts, effectiveConfig.NetworkConfig.MaxRetries+extraAttempts, model, retryMsg)
			...
			ctx.AppendRoutingEngineLog(schemas.RoutingEngineCore, schemas.LogLevelInfo, fmt.Sprintf("Retry %d/%d for %s/%s (previous attempt failed: %s%s)", attempts, effectiveConfig.NetworkConfig.MaxRetries+extraAttempts, providerKey, model, routingErrorSummary(bifrostError), keyNote))
		...
		if lastWasPerKeyFailure && keyProvider != nil && attempts < effectiveConfig.NetworkConfig.MaxRetries+extraAttempts {
			if trail, ok := ctx.Value(schemas.BifrostContextKeyAttemptTrail).([]schemas.KeyAttemptRecord); ok && len(trail) > 0 {
