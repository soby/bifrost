...
func (p *PrometheusPlugin) PostLLMHook(ctx *schemas.BifrostContext, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostResponse, *schemas.BifrostError, error) {
	...
	if isStreamFinal {
		...
		// SDK caller — no transport hooks, so this window is all there is. Recorded
		// synchronously so goroutine scheduler delay and the metric work below never
		// inflate the observed overhead.
